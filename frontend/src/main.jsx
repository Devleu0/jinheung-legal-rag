...
function App(){
  ...
  return <main><header><span className="brand">진흥 / LEGAL EVIDENCE</span><button onClick={clear} disabled={busy}>대화 삭제</button></header>
    ...
    {source&&<dialog open aria-labelledby="source-title"><button autoFocus onClick={()=>setSource(null)}>닫기</button><h2 id="source-title">{source.title}</h2><p>{source.locator}</p><p>{source.text}</p><small>{source.kind==='demo'?'시연 자료 작성일':'시행일'}: {source.effective_date} / 수집일: {source.retrieved_at}</small><p><a href={source.url} target="_blank" rel="noopener noreferrer">공식 원문 확인 ↗</a></p><small>인용 검사는 문자열 일치만 보장합니다. 법적 적용과 최신성을 보장하지 않습니다.</small></dialog>}
  </main>;
}
